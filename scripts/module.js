...
Hooks.once("ready", async () => {
  Hooks.on(
    ...
    async (_journalEntryPage, html, info) => {
  ...
  Hooks.on("renderJournalEntryPageTextSheet", async (app, html) => {
    if (!game.settings.get(MODULE_ID, "insert-journal")) return;
    const journalpage = app.document;

    const uuid = journalpage?.uuid;
    const text = await getJournalEntryContent(uuid);
    if (!(html instanceof HTMLElement)) html = html[0]; // v1/v2 interoperability
    if (text) {
      const content = await foundry.applications.ux.TextEditor.enrichHTML(text);
      $(html)
        .find(".journal-page-content")
        .append($(content));
      // $(html[2]).append($(content));
...
async function getJournalEntryContent(journalPageUUID) {
  ...
  switch (journalPageUUID) {
    // A
    case "JournalEntry.kDJSn9wbVvjnqYEj.JournalEntryPage.x2NtFguDC8zATVxs": // A
      journal.page = "A: Gauntlight Ruins";
      break;

    // B
    case "JournalEntry.uMZ0RiCcPSqsx1Nm.JournalEntryPage.Jyoads7l6vDaxZyb": // B
    ...
    // C
    case "JournalEntry.KeWKr2yd8dbppeS5.JournalEntryPage.b75DStTT45gWbHHu": // C
    ...
    // E
    case "JournalEntry.JT0zCcIwsSpKnbB5.JournalEntryPage.i0qZbjkOeAyfCkfZ": // E
    ...
    // F
    case "JournalEntry.KI33EAGPVHP4zyFZ.JournalEntryPage.HspvTIqCvN1zvdso": // F
      journal.page = "F: Labs";
      break;
    ...
    // G
    case "JournalEntry.OyNcqrpRClyhr8BJ.JournalEntryPage.8V5lwumIxcPPnYMP": // G
    ...
    // H
    case "JournalEntry.FwWmtOBFHhvM2gtf.JournalEntryPage.1XuJhsT16oGbuLc8": // H
      journal.page = "H: Farm";
      break;
    ...
    // I
    case "JournalEntry.1vlzl3NJsftNB9Dc.JournalEntryPage.LYgvrg4SI95a4KqP": // I
...
function splitTextAtHeader(headerName, htmlText) {
  ...
  const headerLevel = Number.parseInt(headerMatch[1]);
